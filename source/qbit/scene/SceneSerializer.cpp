...
#include "Qbit/qcs/ScriptEngine.h"
...
namespace YAML {
	...
	template<>
	struct convert<Qbit::UUID>
	{
		static Node encode(const Qbit::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Qbit::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)             \
	...
	static std::string RigidBody2DBodyTypeToString(Rigidbody2DComponent::BodyType bodyType)
	{
		...
		QB_CORE_ASSERT(false, "Unknown body type");
		return {};
	}
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		{
			QB_CORE_ERROR("Failed to load .qbit file '{0}'\n     {1}", filepath, e.what());
			return false;
		}
					...
					if (scriptFields)
					{
						...
						if (entityClass)
						{
							...
							for (auto scriptField : scriptFields)
							{
								...
								QB_CORE_ASSERT(fields.find(name) != fields.end());
